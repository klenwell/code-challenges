...
class BusinessActivityService:
    ...
    def fetch_activities(self, start_date=None, end_date=None):
        ...
        meeting_activities = [BillableEvent.from_calendar_event(meeting) for meeting in meetings]
        email_activities = [BillableEvent.from_email_message(email) for email in emails]

        activities = meeting_activities + email_activities
        return sorted(activities, key=lambda a: a.started_at)

    def fetch_emails(self, start_date=None, end_date=None):
